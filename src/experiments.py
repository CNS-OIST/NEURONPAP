...
class plotFigures:
    ...
    def plotHeatmap(self, results, tag="", divedend=1, Kir=True, stdLabels=False):
        ...
        for PAPattr in ["vPAP", "vSoma"]:
            ...
            _, cbarMax = gl.clim_volt
            plt.colorbar(
                label=gl.d_volt_short, ticks=np.arange(0, cbarMax, 2), extend="max"
            )
            plt.clim((0, cbarMax))
            if stdLabels:
                self.setLabelColors(
...
class procedure(plotFigures):
                ...
                for j in range(len(AllCells[i])):
                    ...
                    if id == 2 and j == len(AllCells[i]) - 1:
                        ...
                        if self.NKA:
                            kw_args["finalize"][
                                0
                            ] = f"{NT_name} Channel Density x{NTChannelComp[0]}"
                            kw_args["finalize"][
                                1
                            ] = f"{NT_name} Channel Density x{NTChannelComp[1]}"
    ...
    def uptakeRatio(self):
        ...
        if rank == 0:
            ...
            _, ax = plt.subplots(figsize=gl.figsize_halfh)
    ...
    def combine_distance_analysis_plots(self):
        ...
        if found_files == 3:
            ...
            cutoff = 9
            gs = fig.add_gridspec(
                nrows=cutoff + emptyrow + 1, ncols=6, hspace=0.5, wspace=0.5
            )
            axes = []
            total_current = [0] * 3
            ax_inset_select = []
                ...
                if cell_id == 0:
                    ...
                    for i in range(2):
                        ax_inset_select[i][0].set_ylabel(
                            gl.clampI + " " + gl.unit_pA, labelpad=-10
                        )
            ...
            fig.text(
                left - 0.11,
                center,
                gl.free("Normalized Electrode Current I/min(I$_{channel}$)"),
                fontsize=plt.rcParams["axes.labelsize"],
                ha="center",
                va="center",
                rotation=90,
